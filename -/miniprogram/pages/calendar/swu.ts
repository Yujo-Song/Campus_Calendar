...
Page({ 
  ...
  ReadInput: function (e) {
    ...
    let number = 0; //设置结果数量
    // let name=this.data.searchID;
    var end="";
    if(this.data.searchID==""){   //如果输入为空
      this.setData({
        ans:"请输入查询日程"
      })
    }
    ...
    for (let i = 0; i < this.data.eventsArray.length; i++) {
      let event_name=this.data.eventsArray[i][1];
      if(event_name.includes(this.data.searchID))
      {
        number += 1;
        end=end+"时间: "+this.data.eventsArray[i][0]+" 日程: "+event_name+"\n";
        this.setData({
          ans:"搜索结果为：" + number + "\n" + end
        })
      }
